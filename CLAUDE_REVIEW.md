...
| #1a `/hidden` page | **fixed** — page deleted, lists generated by `_reporting/parse_yaml_emails.py`; no address but JT own remains in the built site | `aae6087` |
| #1b emails in the public repo | **open by decision** — 78 addresses still in the tracked `_data/mentoring.yml` | — |
...
`hidden.md` rendered every email in `_data/mentoring.yml` into a page published at `https://jthaler.net/hidden`: HTTP 200, 79 unique addresses, no `noindex`, and `robots.txt` a 404. It was "hidden" only in that nothing linked to it — and unlinked is not private. Anything reachable by URL gets found through address-bar autocomplete, referrer headers, browser sync, and crawlers that specifically hunt unlinked pages.

The page is deleted. `_reporting/parse_yaml_emails.py` now generates the lists on demand, following the pattern `parse_yaml_reporting.py` already established, writing to a gitignored `email_lists.txt`. The built site now contains no address other than JT's own, where one page previously carried 79. The `current_year = 2025` hardcode is gone with it, so the "Remaining"/"Done" lists compute against the real year again.

Verified by comparing the script against the last rendered page: the three year-independent lists — Current, Full, Missing — match exactly at 5, 78 and 2 entries.

#### 1b. `_data/mentoring.yml` in the public repo — **still open, by decision**

This is the larger of the two and is unchanged:

```bash
curl -sI https://raw.githubusercontent.com/jdthaler/jdthaler.github.io/main/_data/mentoring.yml
```

78 `email:` lines, publicly readable as raw YAML, indexed by GitHub code search, present since **January 2023** across **108 commits**. It is the *more* harvestable form than the rendered page ever was: one labelled address per line, no HTML to strip.

Composition, checked without printing addresses: **29 gmail.com**, 19 mit.edu, 30 other institutions. So this is not mostly institutional listings already in MIT's directory — over a third are personal accounts belonging to former students and postdocs.

JT elected to leave this as is for now. Recording it rather than closing it, with what a fix would involve:

- `person.email` is read by `hidden.md` alone — now deleted — so **nothing on the site needs these addresses any more.** They could move to a gitignored local data file with no effect on `/group`, `/cv`, `/join`, or `_cv/parse_yaml.py`. That makes the fix cheaper than it looks.
- Cleaning HEAD would not clean the 108 commits of history; that needs `git filter-repo` and a force-push.
- Be realistic about the ceiling: after 3.5 years public, anything already scraped stays scraped. The repo has 0 forks, which is mildly reassuring but not proof. The achievable goal is stopping further exposure, not erasure.
